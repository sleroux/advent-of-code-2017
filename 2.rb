...
for r in 0..15 do
    ...
    for c in 0..15 do
        index = 16 * r + c 
        value = numbers[index].to_i
        for value_ahead_index in (c+1)..15 do
            value_ahead = numbers[16 * r + value_ahead_index].to_i
            if value % value_ahead == 0 then 
                divide_result = value / value_ahead 
                break
            end
            if value_ahead % value == 0 then 
                divide_result = value_ahead / value 
                break
            end
        end
    end
